...
//
// Badge trạng thái tái sử dụng cho quán, đơn hàng, và các entity khác.
// Dùng trong: store_dashboard_screen, my_stores_screen, store_screens,
//             home feed card, order list, v.v.
//
// Usage:
//   StatusBadge.store(status: getStoreStatus(store, now))
//   StatusBadge.order(status: 'pending')
//   StatusBadge(label: 'Tuỳ chỉnh', color: Color(0xFF...))

import 'package:flutter/material.dart';

// ─────────────────────────────────────────────────────────────────────────────
// Enums (mirror helper getStoreStatus — module 02 spec)
// ─────────────────────────────────────────────────────────────────────────────

enum StoreStatus { open, preorder, emergencyClosed, suspended }

/// Trạng thái đơn hàng — sẽ mở rộng ở module 05
enum OrderStatus {
  pending,
  confirmed,
  preparing,
...
class StatusBadge extends StatelessWidget {
  ...
  /// Hiện dấu chấm bên trái label (mặc định true)
  final bool showDot;

  /// Dấu chấm nhấp nháy — dùng cho trạng thái "đang hoạt động" (mặc định false)
  final bool pulseDot;
  ...
  // ── Factory: từ string (dùng khi nhận từ API response) ─────────────────
