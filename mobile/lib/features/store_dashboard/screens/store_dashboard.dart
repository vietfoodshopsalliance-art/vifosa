  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return storesAsync.when(
      ...
      data: (stores) {
        ...
        // Auto-redirect: prefer last visited store, fallback to first
        WidgetsBinding.instance.addPostFrameCallback((_) async {
          if (!context.mounted) return;
          final prefs = await SharedPreferences.getInstance();
          final lastId = prefs.getString('last_store_id');
          final target = (lastId != null && stores.any((s) => s.id == lastId))
              ? lastId
              : stores.first.id;
          if (context.mounted) {
            context.replace('/store-dashboard/$target/orders');
          }
        });
