...
// ─────────────────────────────────────────────────────────────────────────────
// lib/features/cart/providers/cart_provider.dart  (stub cho module 05)
// ─────────────────────────────────────────────────────────────────────────────
...
class CartNotifier
    extends StateNotifier<Map<String, CartItem>> {
  CartNotifier() : super({});
  ...
  void remove(String itemId) {
    ...
    if (existing.quantity <= 1) {
      final newState = Map<String, CartItem>.from(state)..remove(itemId);
      state = newState;
    } else {
      state = {
        ...state,
...
final cartProvider =
    StateNotifierProvider.family<CartNotifier, Map<String, CartItem>, String>(
  (ref, storeId) => CartNotifier(),
);

final cartItemQtyProvider = Provider.family<int, String>((ref, itemId) {
  // Cần biết storeId — trong thực tế inject từ context
  return 0;
});
